...
export async function POST({ request, cookies }) {
	try {
		...
		const user = new User({ name: name.trim(), email: email.toLowerCase().trim(), password, role: 'user' });
		await user.save();

		const session = { id: user._id.toString(), name: user.name, email: user.email, role: user.role as 'admin' | 'user' };
